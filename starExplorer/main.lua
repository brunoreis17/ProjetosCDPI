...
local sprite = graphics.newImageSheet ("sprite.png", spriteOpcoes)

local backGroup = display.newGroup ()
local mainGroup = display.newGroup ()
local UIGroup = display.newGroup ()

local bg = display.newImageRect (backGroup,"bg.png", 800, 1400)
...
local function moverNave (event)
    ...
    if ("began" == phase) then
-- define a nave como foque de toque.
...
local function gameLoop ()
    ...
    for i = #asteroidesTable, 1, -1 do
        ...
        if (thisAsteroide.x < -100 or thisAsteroide.x > display.contentWidth + 100 or thisAsteroide.y > display.contentHeight + 100) then
